...
**Current state:** `bin/mux` now auto-saves on mux-managed session entry, `mux`, `mux help`, `mux h`, `mux -h`, and `mux --help` print usage, `mux init [claude|codex] [--scope ...]` installs Claude and Codex hook config for supported agent CLIs while printing the touched paths and skipping missing apps, `mux uninstall [claude|codex] [--scope ...]` removes mux-managed hook entries without clobbering unrelated settings and defaults Claude uninstalls to sweeping user/project/local-project scopes, `mux list` and `mux l` expose numeric and letter selectors and append live tmux sessions that are not in the mux snapshot, `mux join` and `mux j` are the only selector-entry commands and resolve existing combined list entries without fallback, bare `mux 1` and `mux b`-style tokens are invalid and print usage, `mux join` and `mux j` prompt interactively when no selector is given, `mux t <name>` and `mux tab <name>` are the explicit session-launch commands, `mux save` and `mux s` rewrite the snapshot on demand, `mux restore` and `mux r` best-effort restore saved `mux <name>` terminals, mux-managed terminals use the canonical `mux <name>` title format for save and restore, `mux cleanup` removes orphan tmux sessions from the live `cmux` host after confirmation, `bin/install-claude-sandbox` can scaffold a bare-bones Claude Docker sandbox with CMUX notification delivery, a host-backed `bd` shim, optional extra allowed host ports, and optional extra read-only mounts into another project, remote shells can still use `mux` without `cmux`, and `README.md` documents the current CLI behavior.
